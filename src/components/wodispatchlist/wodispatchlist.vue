  ...
  <v-data-table :headers="headers" :items="sawflags.items" dense  class="elevation-1"  :search="search"
    ...
    <template v-slot:top >
        ...
        <v-toolbar flat dark dense color="blue darken-4">
            <v-toolbar-title>WO Dispatch List</v-toolbar-title>
    ...
    data() { return {dialog: false,search: '',dialogDelete: false, loading:false, //jobtypeoptions1:[],
      ...
      editedIndex: -1, sawflags:[],// inputRules:[v=>v.length>=3||'Min len is 3 chars'],
              ...
              { text: 'WorkAreaCode', align: 'left',  value: 'WorkAreaCode', width:"1%"},
               ...
               { text: 'ItemDescription', value: 'ItemDescription', sortable: false , width:"1%"},
                { text: 'OperationName', align: 'left',  value: 'OperationName', width:"1%"},
              { text: 'DispatchState', value: 'DispatchState', sortable: false , width:"1%"},
...
//------------------------------------
          ...
          //----------------------------------
            ...
            this.loading=true;
             ...
             axios.get(`/take5/getwodispatchlist?offset=${e.pageStart}`,
             //{params:{'per_page':e.itemsPerPage}}
             )
                    .then((res) => { console.log('pagi-getwodispatchlist response',res.data.items)  
              ...
              if(x.length>2){ this.loading=true;
                 //axios.get(`${axios.defaults.baseURL}/take5/getjobs?search=${x}`)
                  axios.get(`/take5/getwodispatchlist?search=${x}`)
              ...
              if(x.length<=0){ this.loading=true;
                 axios.get(`/take5/getwodispatchlist`)
