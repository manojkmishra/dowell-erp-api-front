<template>
  <v-data-table  :headers="headers" :items="adminuser"  class="elevation-1" >
    <template v-slot:top>
            ...
            <v-card-text>
              <v-container>
                ...
                <!--  <v-col cols="12" sm="6" md="6">
                  ...
                  <v-col cols="12" sm="6" md="6">
                    <v-text-field v-model="editedItem.shop" label="Shop No." :rules="requiredRules"></v-text-field>
                  </v-col>
                </v-row>
                </v-form>
              </v-container>
    ...
    <template v-slot:item.type1="{ item }">
        <span v-if="item.type ==1">Admin</span>
        <span v-if="item.type ==2" >Wholesaler</span>
        <span v-if="item.type ==3">Buyer</span>
    </template>
  ...
  export default {
    data: () => ({
      ...
      headers: [
                        ...
                        { text: "TYPE", align: "left", sortable: true, value: "type1" },
                         { text: "SHOP", align: "left", sortable: true, value: "shop" },
      ...
      desserts: [],categories: [],
      editedItem: { name: '', email: '', type:'',   password: '', confirm_password: '', mobile:'',shop:''},
      editedIndex: -1,
      typeOptions: [ "Admin",  "Wholesaler", "Buyer" ],
...
/*created(){ this.loading=true;
    ...
    computed: {
      formTitle() {  if (this.dialogDelete) { return "Delete User";} 
         ...
         ...mapState({  sawstatus:state => state.user.userlist}),
          adminuser(){let bb= this.sawstatus.filter( x => x.type ===  "2" );
    ...
    methods: { 
      ...
      //  this.editedItem=item;
       // if(this.editedItem.type==1) this.editedItem.type="Admin";
       // else if(this.editedItem.type==2) this.editedItem.type="Wholesaler";
       // else if(this.editedItem.type==3) this.editedItem.type="Buyer";
      ...
      {  //console.log('save-item=',item);
        if (this.editedIndex > -1) //save clicked when editing
              {  console.log('edit',this.editedItem)
                    //edit api here
                    this.editedItem.type=2; //wholesaler
                     // if(this.editedItem.type=="Admin") this.editedItem.type=1;
                    //  else if(this.editedItem.type=="Wholesaler") this.editedItem.type=2;
                     // else if(this.editedItem.type=="Buyer") this.editedItem.type=3;
